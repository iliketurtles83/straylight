...
- [x] Corpus has ≥ 50 utterances, ≥ 10 per class, covers `none/weather` boundary; accuracy ≥ 90% on held-out 20% *(exemplars.jsonl seeded with 50 labeled utterances across `weather`/`none`/`code`/`music`/`system`; `scripts/score_corpus.py` enforces the ≥ 90% gate against a labeled corpus + in-process nomic-embed)*
- [x] Fast/slow path split working: weather utterances hit fast path; conversational utterances hit slow path
- [x] `WeatherSkill` stub plays a spoken response on weather intent
...
- [x] Fast path adds < 100ms over Phase 1 baseline (nomic-embed + spaCy measured separately) *(embed call wrapped in `asyncio.to_thread`; `classifier_ms` logged per turn for ongoing validation)*
- [x] `history_tokens` cap enforced; token count logged per turn via llama.cpp `/tokenize` endpoint *(`ConversationWindow` trims by turn; live `context_tokens` reported via `/tokenize` each turn)*
