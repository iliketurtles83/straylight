...
**Current status (reviewed May 30, 2026):** The main AgentProcessor wiring is in place, but Phase 2 is not fully closed. Token-budget conversation trimming is implemented in `AgentProcessor`; remaining hardening: prove the router corpus/accuracy gate with real Whisper transcripts, validate `/tokenize` at startup, separate heuristic routing from real embedding confidence in diagnostics, clean up prompt loading, make wake ack playback dependency explicit, and unwind the temporary direct Open-Meteo weather adapter into the Phase 3 MCP shape.
...
- `/tokenize` is a Phase 2 dependency, not just UI trivia. Runtime trimming now falls back to a conservative character/word estimate if `/tokenize` fails, so history still fails closed. Remaining hardening: startup validation should probe `POST /tokenize`, and future streaming integrations should prefer server-provided `usage` fields where available.
...
- [ ] `history_tokens` cap enforced by token budget, not turn count; fallback estimate and caching defined; `/tokenize` availability still needs a startup check
