#!/usr/bin/env bash
# Straylight Gemma dev launcher.
#
# Usage:
#   bash scripts/dev_gemma.sh                  (reads .env, wake word mode)
#   bash scripts/dev_gemma.sh --listen         (skip wake word, always-on STT)
#   bash scripts/dev_gemma.sh --no-validate    (skip startup asset checks)
#   bash scripts/dev_gemma.sh --listen --no-validate
#   LLAMA_MODEL=... bash scripts/dev_gemma.sh
#
...
: "${LLAMA_CTX_SIZE:=32768}"  # context window
...
if [[ "${CASS_SKIP_LLM_SERVER_START}" == "true" ]]; then
  ...
  echo "  port:    ${LLAMA_PORT}  gpu-layers: ${LLAMA_GPU_LAYERS}"
  ...
  "${LLAMA_SERVER}" \
    ...
    &
  LLAMA_PID=$!
  echo "[dev_gemma.sh] llama-server PID=${LLAMA_PID}"
fi
...
cleanup() {
  ...
  if [[ -n "${VOICE_PID}" ]]; then
    echo "[dev_gemma.sh] stopping voice service (PID=${VOICE_PID})"
    kill "${VOICE_PID}" 2>/dev/null || true
    wait "${VOICE_PID}" 2>/dev/null || true
  fi

  if [[ -n "${LLAMA_PID}" ]]; then
    echo "[dev_gemma.sh] stopping llama-server (PID=${LLAMA_PID})"
...
on_signal() {
  local sig="$1"
  echo "[dev_gemma.sh] received ${sig}; shutting down"
  cleanup
  exit 130
}
...
# --------------------------------------------------------------------------
echo "[dev_gemma.sh] waiting for ${HEALTH_URL} (max ${LLAMA_HEALTH_TIMEOUT}s — large models take a while)..."
elapsed=0
until curl -sf "${HEALTH_URL}" > /dev/null 2>&1; do
  ...
  echo "[dev_gemma.sh] still loading... ${elapsed}s / ${LLAMA_HEALTH_TIMEOUT}s"
done
echo "[dev_gemma.sh] llama-server healthy after ${elapsed}s"

# --------------------------------------------------------------------------
# Start the voice service
# --------------------------------------------------------------------------
echo "[dev_gemma.sh] starting voice service (args: ${VOICE_ARGS[*]:-none})"
export PYTHONPATH="${REPO_ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
cd "${REPO_ROOT}"
"${VENV}" -m services.voice.main "${VOICE_ARGS[@]}" &
VOICE_PID=$!
